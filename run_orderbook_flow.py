...
from strategy.orderbook_strategy.orderbook_flow.run import OrderbookFlowStrategyRun
...
def run(dryrun, forcerun, broker_str):
    logging.set_log_name('us_orderflow_trading')
    ...
    trade_run = OrderbookFlowStrategyRun(
        dryrun,
        config.get_positionsize(cfg),
        subscription_id = os.getenv('ORDERBOOK_STREAM_FLOW_TRADING_PUBSUB_SUBSCRIPTION_ID'),
