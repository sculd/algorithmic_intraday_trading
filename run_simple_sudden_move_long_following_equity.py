...
from strategy.bar_with_times_strategy.simple_sudden_move.long_trend_following.equity.run import SimpleSuddenMoveLongTrendFollowingTradeEquityStrategyRun
...
def run(dryrun, forcerun, broker_str):
    logging.set_log_name('us_intraday_trading_simple_sudden_move')
    cfg = config.load('config.us.simple_sudden_move.reversal.yaml')
    tz = config.get_tz(cfg)
    logging.info("reading pubsub from {id}".format(id=os.getenv('FINANCE_STREAM_INTRADAY_SIMPLE_SUDDEN_MOVE_TRADING_PUBSUB_SUBSCRIPTION_ID')))

    trade_run = SimpleSuddenMoveLongTrendFollowingTradeEquityStrategyRun(
        dryrun,
        config.get_positionsize(cfg),
        subscription_id = os.getenv('FINANCE_STREAM_INTRADAY_SIMPLE_SUDDEN_MOVE_TRADING_PUBSUB_SUBSCRIPTION_ID'),
