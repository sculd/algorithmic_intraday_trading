...
from strategy.factors.orderbook_factors.order_imbalance_ratio import OrderImbalanceRatio, OrderImbalanceRatioParameter

_MINIMUM_SHORT_POSITION_HOLD_SECONDS = 5
_MINIMUM_LONG_POSITION_HOLD_SECONDS = 5
_SHORT_POSITION_TIMEOUT_SECONDS = 20
_LONG_POSITION_TIMEOUT_SECONDS = 20

def get_voi_factor_param():
    param = VolumeOrderImbalanceParameter()
    return param

def get_oir_factor_param():
    param = OrderImbalanceRatioParameter()
    return param

class OrderbookFlowStrategyParameter:
    def __init__(self,
...
def get_strategy_param():
    param = OrderbookFlowStrategyParameter(
...
class OrderbookFlowStrategy(OrderbookTradeStrategy):
    def __init__(self, positionsize, symbol, long_enter, long_exit, short_enter, short_exit, current_time = None):
        ...
        factor_map = {
            'voi': VolumeOrderImbalance(symbol, self.orderbook, get_voi_factor_param()),
            'oir': OrderImbalanceRatio(symbol, self.orderbook, get_oir_factor_param())
        }
    ...
    def update_position_mode_on_ingest(self):
        ...
        oir = self.factor_map['oir'].get()

        if self.position_mode is POSITION_MODE.NO_POSITION:
            if voi > 500 and oir > 0.8:
                new_position_mode = POSITION_MODE.LONG_ENTERED
                util.logging.info("For {symbol}, long enter signal was observed, voi: {voi}, oir: {oir}, current_price: {cp}".format(
                    symbol=self.symbol, voi=voi, oir=oir, cp=cp))
            elif voi < -500 and oir < -0.8:
                new_position_mode = POSITION_MODE.SHORT_ENTERED
                util.logging.info("For {symbol}, short enter signal was observed, voi: {voi}, oir: {oir}, current_price: {cp}".format(
                    symbol=self.symbol, voi=voi, oir=oir, cp=cp))
