from strategy.orderbook_strategy.volume_order_imbalance.strategy import VolumeOrderImbalanceStrategy
import util.logging
...
from strategy.orderbook_strategy.run_csv import OrderbookStrategyCsvRun

class VolumeOrderImbalanceStrategyCsvRun(OrderbookStrategyCsvRun):
    def __init__(self, positionsize, csv_filename):
        ...
        symbol_to_strategy = lambda symbol: VolumeOrderImbalanceStrategy(self.positionsize, symbol, self.long_enter, self.long_exit, self.short_enter, self.short_exit, current_time=mock_current_time)
