...
from strategy.bar_with_times_strategy.timeseries_analysis.trend_reserval.equity.strategy import SuddenMoveTrendReversalEquityTradeStrategy
import util.logging
...
class SuddenMoveTrendReversalTradeStrategyRun(BarWithTimeTradeStrategyRun):
    def __init__(self, dry_run, positionsize, a_current_time = None, subscription_id = None, equity_broker_mode=EQUITY_BROKER_MODE.TDAMERITRADE):
        ...
        symbol_to_strategy = lambda symbol: SuddenMoveTrendReversalEquityTradeStrategy(self.positionsize, symbol, self.long_enter, self.long_exit, self.short_enter, self.short_exit, current_time=self.current_time)
